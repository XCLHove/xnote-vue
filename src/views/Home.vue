...
<template>
    <div class="container">
        ...
        <div class="search-part">
            <el-input v-model="searchText.title" placeholder="搜索标题" @keyup.enter="lockSearchNote"/>
            <el-input v-model="searchText.keywords" placeholder="搜索关键词" @keyup.enter="lockSearchNote"/>
            <el-input v-model="searchText.content" placeholder="搜索笔记内容" @keyup.enter="lockSearchNote"/>
        ...
        <div class="table-data" v-loading="loading">
            <el-table :data="page.list" :height="tableHeight">
                ...
                <el-table-column prop="isPublic" label="是否公开" width="80">
                    <template #default="scope">
                        <el-tag :type="scope.row.isPublic === NoteIsPublic.YES ? 'success' : 'danger'">{{ scope.row.isPublic }}</el-tag>
                    </template>
                </el-table-column>
            </el-table>
        </div>
