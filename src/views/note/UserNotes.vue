...
import {deleteNoteById, getNoteById, pageSelfNote, updateNote} from "../../api/NoteApi.ts";
...
async function searchNote() {
    ...
    await pageSelfNote(notePageDTO, (result: Result<NotePageDTO>) => {
        page.value.total = result.data.total ? result.data.total : page.value.total
        ...
        <div class="user-notes" v-loading="loading">
            <el-table :data="page.list" style="width: 100%" :height="tableHeight">
                ...
                <el-table-column prop="isPublic" label="是否公开" width="80">
                    <template #default="scope">
                        <el-tag :type="scope.row.isPublic === NoteIsPublic.YES ? 'success' : 'danger'"
                ...
                <el-table-column fixed="right" label="操作" width="150" class="operation">
                    <template #default="scope">
